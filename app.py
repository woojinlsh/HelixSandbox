...
with col_sys1:
    api_key = st.text_input("Verkada API Key", type="password", help="Command에서 발급받은 최상위 API Key")
...
with col_data1:
    ...
    price = st.number_input("Price", min_value=0, value=5000, step=100)

with col_data2:
    payment_type = st.selectbox("Payment Type", ["Credit Card", "Cash", "Mobile Pay"])
    discount = st.text_input("Discount", value="0") 
    discount_percentage = st.number_input("Discount Percentage", min_value=0, max_value=100, value=0, step=1)
...
st.markdown("**전송될 API URL:**")
st.code(api_url, language="http")

st.markdown("**전송될 JSON 데이터 (Payload):**")
st.json(payload)

if st.button("🚀 토큰 발급 및 Helix 메시지 전송", use_container_width=True):
    # 4가지 필수 파라미터가 모두 입력되었는지 검증
    if not api_key or not org_id or not camera_id or not event_type_uid:
        st.warning("⚠️ [1. 연동 정보 설정]의 4가지 항목(API Key, Org ID, Camera ID, Event Type UID)을 모두 입력해주세요!")
    else:
        with st.spinner("🔑 1단계: API 인증 토큰 발급 중..."):
            short_lived_token = get_verkada_token(api_key)
            
        if short_lived_token:
            with st.spinner("📡 2단계: Helix로 POS 데이터 전송 중..."):
                headers = {
                ...
                try:
                    ...
                    if response.status_code == 200:
                        st.success("✅ 전송 성공! (Verkada Command에서 영상을 확인하세요)")
